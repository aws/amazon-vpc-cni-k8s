...
Disable SNAT when pods need to be directly reachable from external networks (VPN, Direct Connect, or other VPCs) without NAT translation, and when pods do not require direct Internet access via an Internet Gateway. When SNAT is disabled, nodes must run in private subnets with outbound Internet connectivity provided through an AWS NAT Gateway or another external NAT device
