...
* In case, the instance has multiple network cards (p5.48xlarge) the max limits is calculated only against the default network card (0). VPC CNI, at the moment, supports assignment only to default network card (index 0). The `MaximumNetworkInterfaces` given in the [vpc_ip_resource_limit.go](https://github.com/aws/amazon-vpc-cni-k8s/blob/master/pkg/vpc/vpc_ip_resource_limit.go) accounts for this limitation with multi-card support. Using prefix delegation is recommended in this situation.
