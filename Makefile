...
unit-test:
	...
	GOOS=linux CGO_ENABLED=1 go test -v -cover ./pkg/controller/eniconfig/...
	GOOS=linux CGO_ENABLED=1 go test -v -cover ./pkg/controller/node/...
	GOOS=linux CGO_ENABLED=1 go test -v -cover ./pkg/controller/pod/...
...
unit-test-race:
	...
	GOOS=linux CGO_ENABLED=1 go test -v -cover -race -timeout 10s ./pkg/controller/eniconfig/...
	GOOS=linux CGO_ENABLED=1 go test -v -cover -race -timeout 10s ./pkg/controller/node/...
	GOOS=linux CGO_ENABLED=1 go test -v -cover -race -timeout 10s ./pkg/controller/pod/...
...
# go vet
vet:
	GOOS=linux go vet ./cmd/manager/
	GOOS=linux go vet ./plugins/routed-eni/
	GOOS=linux go vet ./client/health-check/
