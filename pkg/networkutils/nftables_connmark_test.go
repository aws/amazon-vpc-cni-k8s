...
func TestClassifyRestoreRule(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			rule:       func() *nftables.Rule { return newTestLegacyRestoreRule(mark) },
			mark:       mark,
			expectedOK: false,
		},
		{
...
func TestEnsureBaseChainRulesReplacesLegacyRuleAndUsesReturnedOrder(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
...
func TestEnsureBaseChainRulesDeletesOnlyStaleRules(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			baseChain := &nftables.Chain{Name: nftBaseChainName, Table: table}
			targetChain := &nftables.Chain{Name: nftChainName, Table: table}
			legacyRule := newTestLegacyRestoreRule(mark)
			legacyRule.Handle = 5
			...
			mockNft.EXPECT().DelRule(legacyRule).Return(tt.deleteErr)
