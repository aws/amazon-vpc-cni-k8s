...
func (n *linuxNetwork) UpdateHostSNATRules(vpcCIDRs []string, primaryMAC string, primaryAddr *net.IP,
	...
	var connmarkErr error
	if n.useExternalSNAT {
		connmarkErr = n.connmark.Cleanup()
	} else {
		connmarkErr = n.connmark.Setup(append(vpcCIDRs, n.excludeSNATCIDRs...))
	}
	result := "success"
	if connmarkErr != nil {
		result = "error"
	}
	prometheusmetrics.ConnmarkReconcileCount.WithLabelValues(result).Inc()
	return connmarkErr
}
...
func setupENINetwork(eniIP string, eniMac string, networkCard int, eniSubnetCIDR string, netLink netlinkwrapper.NetLink,
	...
	// Avoid letting the kernel install a transient connected prefix route in the
	// main table. We add the ENI-specific routes explicitly below.
