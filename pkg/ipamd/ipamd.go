...
func (c *IPAMContext) hasRoomForEni(networkCard int) bool {
	trunkEni := 0
	if c.awsClient.IsENITrunkingSupported() && c.enablePodENI && networkCard == DefaultNetworkCardIndex && c.dataStoreAccess.GetDataStore(DefaultNetworkCardIndex).GetTrunkENI() == "" {
		trunkEni = 1
	}
