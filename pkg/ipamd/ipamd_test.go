...
func TestIPAMContext_askForTrunkENIIfNeeded(t *testing.T) {
	...
	_ = m.rawK8SClient.Create(ctx, &fakeNode)

	fakeCNINode := rcscheme.CNINode{
		ObjectMeta: metav1.ObjectMeta{Name: fakeNode.Name},
	}

	err := m.rawK8SClient.Create(ctx, &fakeCNINode)
	assert.NoError(t, err)
	...
	err = m.rawK8SClient.Get(ctx, NodeKey, &notUpdatedNode)
	assert.NoError(t, err)
	var cniNode rcscheme.CNINode

	err = mockContext.rawK8SClient.Get(ctx, types.NamespacedName{
		Name: fakeNode.Name,
	}, &cniNode)
	assert.NoError(t, err)
...
func TestAddFeatureToCNINode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.msg, func(t *testing.T) {
			...
			mockContext.rawK8SClient.Create(ctx, fakeCNINode)
			...
			err = mockContext.rawK8SClient.Get(ctx, key, &wantedCNINode)
			assert.NoError(t, err)
