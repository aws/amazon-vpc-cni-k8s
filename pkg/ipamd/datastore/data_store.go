...
func (ds *DataStore) UnassignPodIPAddress(ipamKey IPAMKey) (e *ENI, ip string, deviceNumber int, interfaces int, err error) {
	...
	// Interfaces Count is 0 means it was not present in the datastore when we restored. A Pod entry can never have 0 interfaces
	if addr.IPAMMetadata.InterfacesCount == 0 {
		addr.IPAMMetadata.InterfacesCount += 1
	}
	...
	return eni, addr.Address, eni.DeviceNumber, addr.IPAMMetadata.InterfacesCount, nil
}
