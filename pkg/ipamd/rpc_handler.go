...
func (s *server) AddNetwork(ctx context.Context, in *rpc.AddNetworkRequest) (*rpc.AddNetworkReply, error) {
	...
	if s.ipamContext.enablePodENI {
		...
		for resName := range limits {
			if strings.HasPrefix(string(resName), "vpc.amazonaws.com/pod-eni") {
				...
				if in.RequiresMultiNICAccess {
					log.Info("SGP pod doesn't support multiple interfaces per pod, falling back to default one interface per pod")
				}
				// Check that we have a trunk
				trunkENI := s.ipamContext.dataStoreAccess.GetDataStore(0).GetTrunkENI()
				if trunkENI == "" {
	...
	if s.ipamContext.enableIPv4 && ipv4Addr == "" ||
		...
		log.Infof("Required MultiNIC access %t, interfaces required %+v", in.RequiresMultiNICAccess, ipsRequired)
